...
- **Prioridade Mobile:** Siga o padrão Mobile-first por padrão em todas as sugestões de estilo (CSS/Tailwind). Não gere media queries para telas grandes ou layouts de desktop a menos que eu solicite explicitamente.
- **React Native Optimization:** Ao sugerir componentes React Native, priorize o uso de `Flexbox` focado em telas de smartphones. Evite propriedades que não possuem suporte consistente entre iOS e Android.
- **PWA Ready:** Para projetos web (Next.js), mantenha o código focado em performance mobile (touch targets de no mínimo 44x44px e carregamento otimizado de fontes/imagens).